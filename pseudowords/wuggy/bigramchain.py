...
class BigramChain(defaultdict):
    ...
    """A dictionary storing the next possible value, given a list of input
    sequences."""

    def __init__(self, plugin_module, data=None, encoding='utf-8',
                 size=100, cutoff=1, token=False):
    ...
    def load(self, datafile, size=100, cutoff=1, token=False):
        ...
        for i, line in enumerate(lines):
            ...
            frequency = float(frequency) if token is True else 1
            frequency = 1
            ...
            if frequency >= cutoff and random.randint(1, 100) <= size:
                for i in range(n):
                    ...
                    if i + 1 < n:
                        ...
                        self[key][next_key] = (self[key].get(next_key, 0)
                                               + sequence.frequency)
            else:
                pass
    ...
    def set_startkeys(self, reference_sequence=None, fields=None):
        ...
        if reference_sequence is None:
            self.startkeys = dict([(key, 0) for key in self.keys()
                                   if key.position == 0])
        else:
            # FIXME: not used
            # reference_values = [reference_sequence[0].
            #                     __getattribute__(field) for field in fields]
            self.startkeys = {}
    ...
    def build_limit_frequencies(self, fields):
        ...
        for key, nextkeys in self.items():
            position, value = key
            subkey_a = (position, tuple([value.__getattribute__(field)
                                         for field in fields]))

            for nextkey, frequency in nextkeys.items():
                position, value = nextkey
                subkey_b = (position, tuple([value.__getattribute__(field)
                                             for field in fields]))
    ...
    def attribute_filter(self, reference_sequence, attribute):
        ...
        if type(reference_sequence[0]) == self.plugin_module.Segment:
            ...
            for key, nextkeys in self.items():
                try:
                    if (key.value.__getattribute__(attribute) ==
